...
import { Bar, Chain, DangerBtn, Empty, Eyebrow, IconBtn, InlineArea, InlineText, NumCell, Page, Section, Sheet, Tools } from '../components/ui'
...
function Cover({ book, size }: { book: Pick<Book, 'title' | 'author' | 'coverUrl'>; size: CoverSize }) {
  ...
  if (book.coverUrl && !broken) {
    return <img src={book.coverUrl} alt="" loading="lazy" onError={() => setBroken(true)} className={`${box} shrink-0 border border-line object-cover`} />
  }
  if (size === 'sm') {
    return (
  ...
  return (
    <span className={`${box} flex shrink-0 flex-col justify-between overflow-hidden border border-line bg-ink-2 p-3 text-left`}>
      <span className={`line-clamp-5 font-medium leading-tight text-paper ${size === 'lg' ? 'text-micro' : 'text-body'}`}>{book.title}</span>
...
function useCoverBackfill(books: Book[]) {
  const missing = books.filter((b) => !b.coverUrl && !coverTried.has(b.id)).map((b) => b.id).join(',')
  useEffect(() => {
    if (!missing) return
    ...
    void (async () => {
      for (const b of useStore.getState().books.filter((x) => !x.coverUrl && !coverTried.has(x.id))) {
        ...
        if (hit?.coverUrl) {
          useStore.getState().updateBook(b.id, { coverUrl: hit.coverUrl, ...(b.totalPages ? {} : { totalPages: hit.totalPages }) })
        }
      }
    })()
...
function Stars({ book, size = 13 }: { book: Book; size?: number }) {
  ...
  return (
    <div className="flex gap-0.5" role="radiogroup" aria-label="Rating">
      {[1, 2, 3, 4, 5].map((v) => (
        <button key={v} type="button" onClick={() => s.updateBook(book.id, { rating: v === book.rating ? null : v })} aria-label={`Rate ${v}`}>
    ...
    <li className="group flex gap-6 border-b border-line py-6 first:pt-0 last:border-b-0">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            <InlineText value={book.title} onChange={(v) => s.updateBook(book.id, { title: v })} ariaLabel="Book title" className="t-page" />
            <div className="mt-1.5 flex flex-wrap items-baseline gap-x-1.5 text-body text-mute">
              <InlineText value={book.author} onChange={(v) => s.updateBook(book.id, { author: v })} ariaLabel="Author" placeholder="Author" className="text-body text-mute" />
        ...
        <div className="mt-3 flex items-baseline gap-1 text-body text-faint">
          <span>p.</span>
          <NumCell value={book.currentPage || null} onChange={(v) => s.updateBook(book.id, { currentPage: v ?? 0 })} ariaLabel="Current page" width="w-12" />
          <span>of</span>
          <NumCell value={book.totalPages || null} onChange={(v) => s.updateBook(book.id, { totalPages: v ?? 0 })} ariaLabel="Total pages" width="w-12" />
          <span className="num">· {pct}%</span>
        </div>

        <div className="mt-5 flex flex-wrap gap-2">
          <button className="btn btn-solid" onClick={() => s.updateBook(book.id, { status: 'finished', currentPage: book.totalPages || book.currentPage })}>
    ...
    <Sheet open onClose={onClose} title={book.status === 'finished' ? 'Read' : book.status === 'queued' ? 'Want to read' : 'Reading'} wide>
      <div className="flex gap-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <InlineText value={book.author} onChange={(v) => s.updateBook(book.id, { author: v })} ariaLabel="Author" placeholder="Author" className="mt-1 text-body text-mute" />
            ...
            {book.status === 'finished' && (
              <>
                ...
                <input
                  ...
                  onChange={(e) => s.updateBook(book.id, { finishedAt: e.target.value || null })}
                />
              </>
            )}
          </div>
      ...
      <div className="mt-6 border-t border-line pt-4">
        <InlineArea value={book.notes} onChange={(v) => s.updateBook(book.id, { notes: v })} ariaLabel="Notes" placeholder="What stuck. Quotes, ideas, what to do about it…" className="!text-micro" />
      </div>
...
function AddBooks({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const add = (m: BookMatch, status: BookStatus) => {
    s.addBook({ title: m.title, author: m.author, totalPages: m.totalPages, coverUrl: m.coverUrl, status })
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Add books" wide>
      <form className="flex gap-2" onSubmit={run}>
        <input className="field min-w-0 flex-1" placeholder="Title, author or ISBN…" value={query} onChange={(e) => setQuery(e.target.value)} autoFocus />
          ...
          {results.map((m, i) => {
            ...
            return (
              <li key={i} className="flex items-center gap-3 border-b border-line py-2.5 last:border-b-0">
                <Cover book={{ title: m.title, author: m.author, coverUrl: m.coverUrl }} size="sm" />
      ...
      {manual && (
        <form
          ...
          onSubmit={(e) => {
            ...
            s.addBook({ title: title.trim(), author: author.trim(), status: 'queued' })
            setTitle('')
            setAuthor('')
          }}
        >
...
export function Books({ label }: { label: string }) {
  ...
  const shelf = useMemo(() => {
    ...
    return [...groups.entries()].map(([k, books]) => ({ key: k, label: k, books }))
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-col gap-10 border-b border-line pb-10 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <div className="mt-4 inline-flex items-baseline gap-1.5 border border-line px-3 py-1.5 text-micro text-mute">
            ...
            <NumCell value={s.bookGoal} onChange={(v) => s.setBookGoal(v ?? s.bookGoal)} ariaLabel="Yearly reading goal" width="w-8" />
        ...
        <div className="grid w-full max-w-md grid-cols-12 gap-1.5 lg:w-[26rem]" aria-label={`Books finished per month, ${year}`}>
          {MONTHS.map((m, i) => (
            <div key={i} className="flex flex-col items-center gap-2">
              ...
              <div className="relative h-20 w-full bg-white/[0.06]" title={`${perMonth[i]} in ${new Date(year, i).toLocaleDateString('en-GB', { month: 'long' })}`}>
                <div
              ...
              <span className={`text-micro ${i === month ? 'font-medium text-paper' : i > month ? 'text-ghost' : 'text-faint'}`}>{m}</span>
            </div>
          ))}
        </div>
      </header>
      ...
      <div className="mt-10 grid grid-cols-[minmax(0,1fr)] gap-x-16 gap-y-14 lg:grid-cols-[minmax(0,1fr)_minmax(0,260px)]">
      ...
      <section className="mt-16">
        ...
        {shelf.every((g) => !g.books.length) ? (
          <Empty>{filter ? 'Nothing matches.' : tab === 'read' ? 'Nothing finished yet — the wall fills itself.' : 'Queue is empty. Add books to line up what’s next.'}</Empty>
        ) : (
          shelf.map((g) => (
